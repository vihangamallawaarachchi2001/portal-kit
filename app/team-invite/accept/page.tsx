    ...
    fetch('/api/settings/team/accept', {
      ...
      .then(async d => {
        if (d.already_accepted) {
          if (d.invite_id) setInviteId(d.invite_id)
          setState('already')
        } else if (d.accepted) {
          if (d.invite_id) {
            setInviteId(d.invite_id)
            // If the user is already signed in, switch to the team workspace and go to dashboard
            await fetch('/api/settings/workspace', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ inviteId: d.invite_id }),
            }).catch(() => {})
            // Check if we were redirected (session exists) or got a 401 (not signed in)
          }
          setState('accepted')
        } else {
          setState('error')
