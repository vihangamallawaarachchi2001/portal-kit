...
export default function NotificationsSettings({ plan, initialPrefs }: NotificationsSettingsProps) {
  ...
  return (
    <div className="flex flex-col gap-6 px-8 pt-8 pb-12">
      ...
      {/* Pro features */}
      <div className="bg-white rounded-md shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-outline-variant/40 bg-surface-container/30 flex items-center justify-between gap-2.5">
...
function NotifRow({
  setting, on, loading, saving, error, locked, onToggle,
}: {
  ...
  return (
    <div className={cn(
      ...
      locked ? 'opacity-60' : 'hover:bg-surface-container/20',
    )}>
      <div className={cn('size-8 rounded-md flex items-center justify-center shrink-0', on && !locked ? 'bg-ds-secondary/10' : 'bg-surface-container')}>
        <Icon className={cn('size-4', on && !locked ? 'text-ds-secondary' : 'text-on-surface-variant')} />
      </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {locked && (
            <span className="text-[10px] text-on-surface-variant/50 font-medium">Requires Pro plan</span>
          )}
          {error && (
      ...
      <button
        ...
        role="switch"
        title={locked ? 'Upgrade to Pro to enable this feature' : undefined}
        className={cn(
          'relative inline-flex h-5 w-9 shrink-0 rounded-full transition-colors duration-200',
          locked ? 'cursor-not-allowed opacity-40 bg-outline-variant/60' : 'cursor-pointer disabled:cursor-not-allowed disabled:opacity-60',
          !locked && (on ? 'bg-ds-secondary' : 'bg-outline-variant/60'),
        )}
      >
        {saving ? (
          <Loader2 className="size-3.5 text-white absolute inset-0 m-auto animate-spin" />
          ...
          <span
            className={cn(
              'pointer-events-none inline-block size-4 rounded-full bg-white shadow-sm transform transition-transform duration-200 mt-0.5',
              on && !locked ? 'translate-x-4' : 'translate-x-0.5',
            )}
          />
        )}
      </button>
